            ...
            while (true)
            {
                try
                {
                    ...
                    while (true)
                    {
                        ...
                        try
                        {
                            LogFileManager.ObjLog.info(System.IO.Directory.GetCurrentDirectory());
                            client.iSetTaskState(TaskID, 1);
                            Filenames = Ti.filenames;
                            TaskID = Ti.TaskID;
                            proname = Ti.ProName;
                            path1 = path + "\\" + proname + "\\";
                            LogFileManager.ObjLog.info(path1);
                            //这里加一个迭代删除文件即可
                            ...
                            LogFileManager.ObjLog.info(paths[0] + "------" + paths[1] + "------" + paths[2] + "------" + paths[3]);
                            for (int i = 0; i < paths.Length; i++)   //下载工程文件
                            {
                                paths[i] = paths[i].Replace("\\\\", "\\");//临时改的。为了调试
                                string data = client.GetData(paths[i]);//异常检查
                                StreamWriter sw = new StreamWriter(path1 + Filenames[i]);
                                sw.Write(data);
                                sw.Close();
                                string[] splitString = { " ", "\r\n" };
                                string[] splitData = data.Split(splitString, StringSplitOptions.RemoveEmptyEntries);
                                for (int j = 0; j < splitData.Length; j++)
                                {
                                    if (splitData[j].Equals("uan_path"))
                                    {
                                        string uanPath = paths[i].Substring(0, paths[i].LastIndexOf('\\') + 1) + splitData[j + 1];
                                        string uanData = client.GetData(uanPath);
                                        StreamWriter uanSW = new StreamWriter(path1 + splitData[j + 1]);
                                        uanSW.Write(uanData);
                                        uanSW.Close();
                                    }
                                }
                        ...
                        catch (System.TimeoutException e)
                        {
                            ...
                            LogFileManager.ObjLog.info("任务失败1");
                        ...
                        catch (FaultException<ServiceReference1.WcfException> e)
                        {
                            ...
                            LogFileManager.ObjLog.info("任务失败2");
                            //Directory.Delete(path1, true);
                            Thread.Sleep(10000);
                            continue;
                        }
                        catch (CommunicationException e)
                        {
                            client.iSetTaskState(TaskID, 0);
                            LogFileManager.ObjLog.debug(e.Message, e);
                            LogFileManager.ObjLog.info("任务失败3");
                            // Directory.Delete(path1, true);
                            Thread.Sleep(10000);
                            continue;
                        }
                        catch (Exception e)
                        {
                            client.iSetTaskState(TaskID, 0);
                            LogFileManager.ObjLog.debug(e.Message, e);
                            LogFileManager.ObjLog.info("任务失败4");
                            //Directory.Delete(path1, true);
                            Thread.Sleep(10000);
                            continue;
                        }
                        ...
                        if (txfile.Length != 0 && setupdir.Length != 0 && rxfile.Length != 0 && terfile.Length != 0)
                        {
                            ...
                            ComputerContral_server CC = new ComputerContral_server(path1, setupdir[0].Name, terfile[0].Name, txfile[0].Name, rxfile[0].Name, path2, exename);
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < resultfile.Length; i++)
                            {
                                ...
                                client.PutData(resultpath + "\\", resultfile[i].Name, data);//先保存在temp文件夹中待合并
                                sr.Close();
                            }
                            //主要修改上面的一段
                ...
                {
                    LogFileManager.ObjLog.error("程序崩溃，原因未知");
                }
            }
        }
