...
{
    public class PlOutput : P2mFileOutput
    {
        ...
        private static void OutputRxPathLoss(Node RxNode, string Rxname, double passloss, double totaldistance)//接收机的名字,加到path里面
        {
            sb.AppendLine("# Receiver Set:" + Rxname);
            sb.AppendLine("#   Rx#      X(m)           Y(m)          Z(m)     Distance (m) Path Loss (dB)");
