...
{
    class EzmagOutput : P2mFileOutput
    {
        public static void OutPath(List<CalculateModelClasses.Path> Paths, string sPath, string projectName, int TxIndex, int txTotol, ReceiveBall rxballtemp, Point txpotemp, int Frequence, int minFrequence, int maxFrequence)//需要提供一个接受区域所有路径的信息
        {
            ...
            //sPath += projectName;
            FileStream fs = new FileStream(sPath + projectName + "_ezmag" + "_t00" + TxIndex + "_0" + txTotol + "_" + Frequence.ToString("D4") + "_" + minFrequence.ToString("D4") + "_" + maxFrequence.ToString("D4") + "_r0" + groupnum + ".p2m", FileMode.Create);//如果是接收区域的话，多个接收点组成一组接收机组成一个编号
            StreamWriter sw = new StreamWriter(fs);
            if (Paths.Count != 0)
            {

                OutputRxEZField(Paths[0].node[Paths[0].node.Count - 1], Rxname, txpotemp.GetDistance(rxballtemp.Receiver), GetTolEz(Paths));
        ...
        {
            FileStream fs = new FileStream(sPath + @"exmagOutputNull.p2m", FileMode.Append);
            StreamWriter sw = new StreamWriter(fs);
            sw.Close();
            fs.Close();
        }

        private static void OutputRxEZField(Node RxNode, string Rxname, double distance, Plural totalEZ)//接收机的名字,加到path里面
        {
            sb.AppendLine("# Receiver Set:" + Rxname);
            sb.AppendLine("#   Rx#      X(m)           Y(m)            Z(m)   Distance    Magnitude    Real        Imag");
            sb.AppendLine("     " + 1 + "  " + RxNode.Position.X.ToString("0.#######E+00") + "  " + RxNode.Position.Y.ToString("0.#######E+00") + "     " + RxNode.Position.Z.ToString("f3") + "       " +
                distance.ToString("f2") + "  " + Math.Sqrt(Math.Pow(totalEZ.Re, 2) + Math.Pow(totalEZ.Im, 2)).ToString("0.00000E+00") + "  " + totalEZ.Re.ToString("0.00000E+00") + "  " + totalEZ.Im.ToString("0.00000E+00"));
        }
    }
}
