...
{
    class EyphsOutput : P2mFileOutput
    {
        public static void OutPath(List<CalculateModelClasses.Path> Paths, string sPath, string projectName, int TxIndex, int txTotol, ReceiveBall rxballtemp, Point txpotemp, int Frequence, int minFrequence, int maxFrequence)//需要提供一个接受区域所有路径的信息
        {
            ...
            //sPath += projectName;
            FileStream fs = new FileStream(sPath + projectName + "_eyphs" + "_t00" + TxIndex + "_0" + txTotol + "_" + Frequence.ToString("D4") + "_" + minFrequence.ToString("D4") + "_" + maxFrequence.ToString("D4") + "_r0" + groupnum + ".p2m", FileMode.Create);//如果是接收区域的话，多个接收点组成一组接收机组成一个编号
            StreamWriter sw = new StreamWriter(fs);
            if (Paths.Count != 0)
            {
                OutputRxEYPhase(Paths[0].node[Paths[0].node.Count - 1], Rxname, txpotemp.GetDistance(rxballtemp.Receiver),GetTolEy(Paths));
        ...
        private static void OutputRxEYPhase(Node RxNode, string Rxname, double distance, Plural totalEY)//接收机的名字,加到path里面
        {
            sb.AppendLine("# Receiver Set:" + Rxname);
            sb.AppendLine("#   Rx#      X(m)           Y(m)            Z(m)   Distance     Phase     Real        Imag");
            sb.AppendLine("     " + 1 + "  " + RxNode.Position.X.ToString("0.#######E+00") + "  " + RxNode.Position.Y.ToString("0.#######E+00") + "     " + RxNode.Position.Z.ToString("f3") + "       " + distance.ToString("f2") + "  " + Math.Atan2(totalEY.Im, totalEY.Re).ToString("0.00000E+00") + "  " + totalEY.Re.ToString("0.00000E+00") + "  " + totalEY.Im.ToString("0.00000E+00"));
        }
    }
}
